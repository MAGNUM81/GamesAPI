...
func TestAthorizationServiceTestSuite(t *testing.T) {
	suite.Run(t, new(AuthorizationServiceTestSuite))
}

func (s *AuthorizationServiceTestSuite) SetupSuite() {
	services.AuthorizationService = services.NewAuthorizationService("../resources/rbac-test.yml")
}

func (s *AuthorizationServiceTestSuite) TestRBAC_Authorize() {
	fmt.Printf("%+v", services.AuthorizationService.GetRbac())
	...
	tests := []struct {
		...
		// As a client
		{
			...
			args: args{
				req: func() *http.Request {
					...
					ctx := context.WithValue(context.Background(), domain.ContextKey("email"), "client.one@email.com")
		...
		// As CS
		{
			...
			args: args{
				req: func() *http.Request {
					req, _ := http.NewRequest("", "http://api.example.com/inquiries", nil)
					ctx := context.WithValue(context.Background(), domain.ContextKey("email"), "cs.one@company.com")
		...
		// As an Ops
		{
			...
			args: args{
				req: func() *http.Request {
					...
					ctx := context.WithValue(context.Background(), domain.ContextKey("email"), "ops.one@company.com")
		...
		// As a manager
		{
			...
			args: args{
				req: func() *http.Request {
					req, _ := http.NewRequest("", "http://api.example.com/inquiries", nil)
					ctx := context.WithValue(context.Background(), domain.ContextKey("email"), "manager@company.com")
	...
	for _, tt := range tests {
		s.T().Run(tt.given, func(t *testing.T) {
			req := tt.args.req()
			got := services.AuthorizationService.Authorize(req.Context(), req.URL, tt.args.role, tt.args.resource, tt.args.endpoint)
			if tt.wantErr {
